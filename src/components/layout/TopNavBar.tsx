...
export const TopNavBar: React.FC<TopNavBarProps> = ({
  ...
  return (
    <header className="sticky top-0 right-0 z-30 w-full lg:pl-64 p-2.5 sm:p-3.5 lg:px-6 lg:pt-3.5 lg:pb-2 transition-all duration-300">
      <div
        className={`w-full mx-auto px-3.5 sm:px-5 h-16 rounded-2xl sm:rounded-3xl backdrop-blur-2xl border transition-all duration-300 flex items-center justify-between shadow-xl ${
          isDark
            ? 'bg-[#141b2d]/85 border-white/10 shadow-black/40 ring-1 ring-white/5'
            : 'bg-white/95 border-slate-200/90 shadow-slate-200/60 ring-1 ring-slate-200/40'
        }`}
      >
        {/* Left Side: Mobile Menu Button & Search */}
        <div className="flex items-center gap-2.5 sm:gap-3.5 w-full max-w-xs md:max-w-md">
          <button
            type="button"
            onClick={onOpenMobileMenu}
            className={`p-2 rounded-2xl lg:hidden transition-all cursor-pointer border ${
              isDark
          ...
          <div className="relative w-full">
            ...
            <input
              ...
              placeholder="Buscar aulas, tomografia, radioproteção..."
              className={`w-full pl-10 pr-4 py-2 rounded-2xl text-xs sm:text-sm outline-none transition-all ${
                isDark
                  ? 'bg-[#0a0e17]/70 border border-white/10 text-[#dfe2ef] placeholder:text-[#869397] focus:border-[#4cd7f6] focus:ring-2 focus:ring-[#4cd7f6]/20'
                  : 'bg-slate-100/90 border border-slate-200 text-slate-800 placeholder:text-slate-400 focus:border-cyan-600 focus:bg-white focus:ring-2 focus:ring-cyan-600/15'
              }`}
            />
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div
            className={`hidden md:flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider border shadow-sm ${
          ...
          {currentUser.role === 'admin' ? (
            onNavigateToTab && (
              <button
                ...
                className={`hidden sm:flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-semibold transition-all cursor-pointer border shadow-sm ${
                  isSupabaseConnected
                    ? isDark
            ...
            <div
              ...
              className={`hidden lg:flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[11px] font-medium border opacity-80 ${
                isSupabaseConnected
          ...
          <button
            ...
            className={`p-2 rounded-2xl transition-all flex items-center justify-center cursor-pointer border shadow-sm ${
              isDark
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-2xl text-xs font-bold transition-all cursor-pointer border shadow-sm ${
              isDark
          ...
          <div className="relative">
            <button
              ...
              className={`p-2 rounded-2xl transition-all relative cursor-pointer border shadow-sm ${
                isDark
            ...
            {showNotifications && (
              <div
                className={`absolute right-0 mt-3 w-80 md:w-96 rounded-3xl shadow-2xl p-4 z-50 border backdrop-blur-2xl animate-fade-in ${
                  isDark ? 'bg-[#141c2e] border-white/15 text-white' : 'bg-white border-slate-200 text-slate-800 shadow-slate-300'
                }`}
              >
                <div className="flex items-center justify-between pb-3 border-b border-slate-200/15 mb-3">
          ...
          <div className="relative">
            <button
              ...
              className={`flex items-center gap-2 p-1 sm:pr-2.5 rounded-full transition-all cursor-pointer border shadow-sm ${
            ...
            >
              ...
              <span className="material-symbols-outlined text-sm text-gray-400 hidden sm:inline">
                expand_more
              </span>
            </button>
            ...
            {showProfileMenu && (
              <div
                className={`absolute right-0 mt-3 w-72 rounded-3xl shadow-2xl p-4 z-50 text-xs border backdrop-blur-2xl animate-fade-in ${
                  isDark ? 'bg-[#141c2e] border-white/15 text-white' : 'bg-white border-slate-200 text-slate-800 shadow-slate-300'
                }`}
              >
                ...
                {onLogout && (
                  <div className="pt-2 mt-2 border-t border-slate-200/10">
                    <button
                      ...
                      className="w-full text-left px-2.5 py-2 rounded-xl hover:bg-red-500/15 text-red-400 font-semibold flex items-center gap-2 cursor-pointer transition-colors"
                    >
